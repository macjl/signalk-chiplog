...
- Merging two entries now keeps a record of the stop between them: a `stopover` line on the surviving passage's timeline, naming the place and its position, at the moment the earlier entry had stopped — previously that information was silently lost once the merge took the later entry's arrival as its own.
