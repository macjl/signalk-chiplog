...
- Merging two entries now keeps a record of the stop between them: a `stopover` line on the surviving passage's timeline, naming the place and its position — previously that information was silently lost once the merge took the later entry's arrival as its own. Dated no earlier than the entry's own arrival reading, so it follows that reading on the timeline rather than the raw, earlier moment detection dated the stop from.
