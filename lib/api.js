...
const { renderExport, writeUsbExport } = require('./export');
...
function registerRoutes(router, { getContext, logError }) {
  ...
  readonly.get(
    '/api/state',
    handle(({ db, detection }) => {
      const { mode, motion, propulsion: under } = detection();
      return {
        activeEntryId:
  ...
  readwrite.post(
    ...
    handle(({ db, now, vesselPosition }, req) =>
      entries.closeEntry(db, entryId(req), { now: now(), position: vesselPosition() })
    )
  );
  ...
  readwrite.post(
    ...
    handle(({ db, now, vesselPosition, observeEvent }, req, res) => {
      ...
      const event = events.createEvent(db, id, input, {
        now: now(),
      ...
      if (event.type === 'manoeuvre' && input.time === undefined) {
        observeEvent(id, event.time);
      }
      res.status(201);
      return event;
    })
  ...
  admin.post(
    ...
    handle(({ db, config, now }) => {
      ...
      return writeUsbExport(db, config.usbExportPath, now());
    })
  );
}
