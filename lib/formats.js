...
function toGeoJson(entry, trackPoints) {
  ...
  return {
    ...
    properties: {
      ...
      // Parallel to coordinates/coordTimes, in SI units like the rest of the
      // API: the passage page's position scrubber (SPEC §4.1) reads these for
      // whichever point is selected, rather than fetching readings one by one.
      readings: trackPoints.map((point) => ({
        sog: point.sog,
        cog: point.cog,
        stw: point.stw,
        tws: point.tws,
