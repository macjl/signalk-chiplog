...
function createReplayJob({
  ...
  async function perform(from, to, signal) {
    ...
    try {
      await history.preload(Date.parse(from), Date.parse(to), (doneToMs) => {
        if (current) {
          current.phase = 'fetching';
          current.now = iso(doneToMs);
        }
      });
      if (current) {
        current.phase = 'replaying';
        current.now = from;
      }
      await runReplay({
        db,
  ...
  return {
    start(from, to) {
      ...
      current = { from, to, startedAt: iso(clock()), now: from, phase: 'fetching', controller };
      lastError = null;
