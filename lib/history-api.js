...
function createHistoryApiHistory({
  ...
  async function values(paths, fromMs, toMs, resolutionMs, aggregates = {}) {
    checkAborted();
    // The InfluxDB 2 provider collates positions separately from other values
    // and requires both result sets to have the same number of rows. Sparse
    // historical data does not guarantee that, so the position gets a request
    // of its own -- equivalent for the store, and safe with every provider.
    const groups =
      paths.includes(POSITION_PATH) && paths.length > 1
        ? [[POSITION_PATH], paths.filter((path) => path !== POSITION_PATH)]
        : [paths];
    const results = [];
