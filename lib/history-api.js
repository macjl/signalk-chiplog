...
function createHistoryApiHistory({ getHistoryApi, selfContext, signal }) {
  ...
  async function values(paths, fromMs, toMs, resolutionMs) {
    checkAborted();
    // The InfluxDB 2 History provider collates positions separately from other
    // values, and requires both result sets to have the same number of rows.
    // Sparse historical data does not guarantee that, so keep the position in
    // its own request. Loading the responses independently is equivalent for
    // the replay cache and works with every History API provider.
    const groups =
      paths.includes('navigation.position') && paths.length > 1
        ? [['navigation.position'], paths.filter((path) => path !== 'navigation.position')]
        : [paths];
    const historyApi = await api();
  ...
  async function preload(fromMs, toMs, onChunk, { bucketMs } = {}) {
    ...
    for (let start = fromMs; start < toMs; start += CHUNK_MS) {
      ...
      for (const result of await values(paths, start, end, bucketMs ?? 1_000)) load(result);
      onChunk?.(end, toMs);
    }
    sortLoaded();
  }
