...
function passageFingerprint(db, row, pdf) {
  ...
  // The weather forecast arrives once, after departure, and is printed.
  const weather = db
    .prepare('SELECT fetched_at FROM weather_forecasts WHERE entry_id = ?')
    .get(row.id);
  hash.update(JSON.stringify(weather?.fetched_at ?? null));
