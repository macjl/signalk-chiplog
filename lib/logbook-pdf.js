...
function loadShared() {
  ...
  sharedModules ??= Promise.all([
    ...
    load('log-lines.mjs'),
    load('weather.mjs')
  ]).then(([i18n, format, lines, weather]) => ({ i18n, format, lines, weather }));
...
function passageLines(bundle, context) {
  ...
  const { entry, events, observations, propulsion, weather } = bundle;
  ...
  // The forecast fetched at departure, as the skipper notes the bulletin
  // before casting off: a line every 3 hours, dated once it runs past the day.
  const forecastText = () => {
    const steps = weather ? context.forecastSteps(weather.points, 3) : [];
    if (steps.length === 0) {
      return [];
    }
    const departureDay = format.dayKey(entry.startTime);
    return [
      { text: t('pdf.forecast'), font: 'bold', newLine: true },
      ...steps.map((step) => ({
        text: context.describeStep(step, {
          t,
          format,
          withDate: format.dayKey(step.time) !== departureDay
        }),
        color: GREY,
        newLine: true
      }))
...
async function renderLogbookPdf(
  ...
  const context = {
    ...
    describeStep: weatherModule.describeStep,
    manoeuvreLabels: {}
  };
