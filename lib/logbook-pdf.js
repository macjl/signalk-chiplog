...
// known for the footers. A day's weather blocks only take up room on its
// first page, above the table -- not repeated on a page it continues onto.
function paginate(days, context) {
  ...
  for (const day of days) {
    ...
    pages.push(page);
    let available = baseAvailable - weatherHeight;
...
async function renderLogbookPdf(
  ...
  // Each passage's weather block belongs on the day it departed, above that
  // day's table rather than mixed into it.
