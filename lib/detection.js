...
function createPassageDetector({ db, readSelfPath, settings, clock = Date.now }) {
  ...
  function sense(now) {
    ...
    const fromState = classifyState(readFresh('navigation.state', now));
    mode = fromState ? 'autostate' : 'fallback';
    return { motion: fromState ?? speeds.motion(now), position: current };
  }
  ...
  function step(now, current, position, previous) {
    ...
    if (entry && entry.stopped_since !== null) {
      if (now - Date.parse(entry.stopped_since) >= closureMs) {
  ...
  return {
    tick() {
      ...
      withTransaction(db, () => step(now, sensed.motion, sensed.position, previous));
      motion = sensed.motion;
      return { mode, motion, activeEntryId: activeEntry()?.id ?? null };
    },
    mode: () => mode,
    motion: () => motion
  };
