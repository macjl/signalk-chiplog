...
function createInfluxHistory({
  ...
  async function scanMotion(fromMs, toMs, { stoppedSpeed }, onChunk) {
    ...
    // Detection follows signalk-autostate whenever it publishes, whatever
    // else does.
    const autostate = stateRows
      .filter((row) => String(row.source ?? '').startsWith(AUTOSTATE_SOURCE_PREFIX))
      .sort((a, b) => a.time - b.time);
    const maxAge = MAX_AGE_MS['navigation.state'];
    autostate.forEach((row, index) => {
      if (!UNDERWAY_STATES.has(row.stringValue)) {
        return;
      }
      const next = autostate[index + 1];
  ...
  function readSelfPath(path, atMs) {
    // Signal K exposes a branch as an object. Recreate the only branch that
    // the replay reads, so historical engine RPMs are actually considered.
    if (path === 'propulsion') {
      const ids = new Set();
      for (const knownPath of series.keys()) {
        const match = /^propulsion\.([^.]+)\./.exec(knownPath);
