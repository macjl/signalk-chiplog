...
function recomputeDurations(db, entryId, now) {
  const totals = db
    .prepare(
      `SELECT type, SUM((julianday(end_time) - julianday(start_time)) * 86400) AS seconds
       FROM propulsion_segments
       WHERE entry_id = ? AND end_time IS NOT NULL
       GROUP BY type`
    )
    .all(entryId);
...
function closeEntry(db, id, { now, position }) {
  return withTransaction(db, () => {
    ...
    const endTime = now < row.start_time ? row.start_time : now;
