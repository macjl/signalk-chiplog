...
- **Reads InfluxDB directly, one day at a time.** `lib/influx-history.js` knows signalk-to-influxdb's schema: one measurement per Signal K path, tagged with context (self) and source (for a path more than one source publishes, `navigation.state` chief among them — resolved the same way the server itself would). Everything the pipeline needs is fetched in daily chunks regardless of the requested range's length, with a short pause between chunks, then answered from memory as fast as the replay loop asks — a single query spanning the whole range would otherwise ask a database for every path across weeks at once, and a boat's InfluxDB often shares its Raspberry Pi with Signal K itself; a fixed-size request keeps each query's result small and gives the database room to recover between them. `GET /replay` reports this as the `"fetching"` phase before replay itself begins (`"replaying"`).
...
- **Every InfluxDB query is bounded to 20 s.** Node's `fetch` has no timeout of its own, so an unreachable or overloaded database would otherwise hang far longer than that for an error no clearer once it arrived — a bare "fetch failed" instead of the actual connection problem.
