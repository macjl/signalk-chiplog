...
The database schema, the REST API, passage detection and track recording exist: the plugin opens and closes logbook entries from live Signal K data and records their track and distance. Not yet built: engine/sail segments, observations, automatic events, and online geocoding (place names only come from already-known places). The documents to read before implementing anything:
