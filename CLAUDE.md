...
The database schema, the REST API, passage detection, track recording, engine/sail detection and instrument snapshots exist: the plugin opens and closes logbook entries from live Signal K data and records their track, distance, propulsion segments and conditions. Not yet built: automatic events (alarms, autopilot, weather thresholds), online geocoding (place names only come from already-known places), the PDF export and any UI. The documents to read before implementing anything:
