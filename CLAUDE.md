...
The database schema, the REST API, passage detection, track recording and engine/sail detection exist: the plugin opens and closes logbook entries from live Signal K data and records their track, distance and propulsion segments. Not yet built: observations, automatic events, and online geocoding (place names only come from already-known places). The documents to read before implementing anything:
